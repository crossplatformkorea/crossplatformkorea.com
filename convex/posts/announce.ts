 ...
 * announcements, as the immediate sends did before this; mention notifications
 * are not repeated.
 */
...
export const announceIfStillPublic = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await notifyMentionedUsers(ctx, post);
    return null;
  },
});

/**
 * In-app notifications for the users a post mentions, from its current
 * `mentions`, so a mention that an edit removed inside the window is not sent.
 *
 * Every publication schedules a check, so a post published, drafted and
 * published again comes through here more than once. A user who already holds
 ...
 */
async function notifyMentionedUsers(ctx: MutationCtx, post: Doc<'posts'>) {
  ...
  for (const userId of post.mentions) {
    const existing = await ctx.db
      ...
      .filter((q) =>
        q.and(
          q.eq(q.field('type'), 'MENTIONED'),
          q.eq(q.field('postId'), post._id),
          q.eq(q.field('commentId'), undefined),
        ),
      )
      .first();
    if (existing !== null) {
  ...
  // first(), not unique(): a duplicate profile row would throw, and the throw
  // would roll back this post's Slack and Discord announcement with it.
