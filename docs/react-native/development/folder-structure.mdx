...
- The `uis` subdirectory is located inside the `app` directory and is the place for reusable components.
  It's not necessary to put all reusable components in this location; they can also be placed within the components of individual screens.
  However, it is advisable to store components that are commonly used across multiple screens here.
- The `utils` subdirectory is where utility functions used in the app are collected.
  This is a collection of necessary functionalities to increase the reusability of code.
