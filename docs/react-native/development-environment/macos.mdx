...
In this section, we've prepared materials to help even those new to development set up their development environment. This is about the `Mac development environment`, so if you're a Windows user, please check out the [Windows development environment](windows).
...
> This sets `Node.js` version `20` in `nvm`.
...
To install OpenJDK 17, which is supported by React Native version 0.63 and later, use this command:
```sh
brew install openjdk@17
```

To add the OpenJDK 17 to your path in the `zshrc` configuration file, enter:
```sh
echo 'export PATH="/opt/homebrew/opt/openjdk@17/bin:$PATH"' >> ~/.zshrc
```

Finally, to apply the changes to your current terminal session, reload the `zshrc` file with:
```sh
source ~/.zshrc
```
...
For a richer and quicker start to a React Native project, we'll use [dooboo-cli](https://medium.com/crossplatformkorea/new-dooboo-cli-5c60e17a87e0), managed by [hyo.dev](https://hyo.dev).
...
Navigate to the folder with the project name, run `bun install` to install necessary local packages, and
...
Then, follow the on-screen prompts to launch your React Native app on the iOS simulator or Android emulator. Also, check out [dooboo-ui](https://dooboo-ui.hyo.dev), an open-source React Native UI kit developed by [hyochan](https://github.com/hyochan) for various components, designs, and utilities.
