...
// 사용자 프로필에 locale 업데이트하는 함수 (컴포넌트에서 호출)
export const updateUserLocale = async (updateProfileMutation: any, locale: 'en' | 'ko' | 'ja') => {
  try {
    await updateProfileMutation({ locale });
  } catch (error) {
...
export const t = (param: NestedKeys<typeof en>, mapObj?: Record<string, any>): string => {
