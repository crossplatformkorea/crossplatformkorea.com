...
만약 프로젝트를 [dooboo-cli](https://github.com/hyochan/dooboo-cli)로 시작했다면 프로젝트 경로에서 `vscode`를 열 때 오른쪽 하단에 추천 플러그인을 설치할 거냐고 물어봅니다. 여기서 만약 설치를 하면 아래 8가지 플러그인들이 자동으로 설치되니 참고해주세요.
