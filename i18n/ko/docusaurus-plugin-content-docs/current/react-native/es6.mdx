...
const fetchData: Promise<string> = new Promise((resolve, reject) => {
  // 비동기 작업 수행 후 성공하면 resolve, 실패하면 reject 호출
  setTimeout(() => {
...
fetchData
  .then((data: string) => console.log(data)) // 성공 시 출력
  .catch((error: string) => console.log(error)); // 실패 시 출력
```
프로미스의 반환 타입을 지정할 수 있습니다. 이 예시에서는 `Promise<string>`으로 선언되었습니다.
...
<details>
  ...
  ```
  콜백 함수를 사용하면 getData 함수가 데이터를 받아온 후에 callback 함수를 실행합니다. 이 방식은 중첩이 많아질수록 코드가 복잡해질 수 있습니다.
</details>
...
### 13. 향상된 객체 리터럴 (Enhanced Object Literals)
```typescript
...
### 15. Number, Math, Array, Object에 새로운 메소드 추가
```typescript
