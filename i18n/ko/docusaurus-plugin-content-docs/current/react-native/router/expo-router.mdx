...
### 3. 헤더 바 설정
헤더의 스타일과 같은 옵션을 전역적으로 설정하려면 `screenOptions` prop을 사용하십시오.
```javascript
...
```
자식 라우트에서 옵션을 동적으로 구성하려면 `<Stack.Screen />` 컴포넌트를 사용하십시오.
```javascript
...
### 6. 사용자 정의 푸시 동작
기본적으로 Stack은 스택에 이미 있는 라우트를 푸시할 때 중복된 페이지를 제거합니다. 라우트의 푸시 동작을 수정하려면 `getId` 함수를 제공하십시오.
```javascript
import { Stack } from 'expo-router';
...
알겠습니다. Expo Router의 Drawer에 대해 한국어로 설명하겠습니다.
...
알겠습니다. Expo Router에서 플랫폼 별로 모듈을 교체하는 방법에 대해 한국어로 설명하겠습니다.
...
같은 URL을 다른 레이아웃과 매치하려면 겹치는 자식 라우트와 함께 그룹을 사용하십시오. 이 패턴은 네이티브 앱에서 매우 일반적입니다. 예를 들어, Twitter 앱에서는 프로필을 모든 탭(홈, 검색, 프로필 등)에서 볼 수 있습니다. 그러나 이 라우트에 액세스하기 위해 필요한 URL은 하나뿐입니다.
